...
- Performance considerations:
  - To avoid retrigering the data fetch on every render, the `useFetch` hook has been designed to only fetch data when explicitly called or when its dependencies change.
  - use of `useCallback`s to memoize functions where appropriate to prevent unnecessary re-renders.
